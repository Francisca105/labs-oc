...
void accessL2(uint32_t address, uint8_t *data, uint32_t mode) {
  ...
  uint32_t offset_bits = get_bits(BLOCK_SIZE);  // log2(BLOCK_SIZE) to calculate the number of bits for offset
  uint32_t index_bits = get_bits(L2_SIZE / BLOCK_SIZE);  // log2(L2_SIZE / BLOCK_SIZE) to calculate the number of bits for index
  ...
  if (!Line->Valid || Line->Tag != Tag) {         // if block not present - miss
    ...
    memcpy(&(L2Cache[index*BLOCK_SIZE + offset]), TempBlock,
           BLOCK_SIZE); // copy new block to L2 line
    Line->Valid = 1;
    Line->Tag = Tag;
    Line->Dirty = 0;

    // copy new block to L1 cache

  } // if miss, then replaced with the correct block

  if (mode == MODE_READ) {    // read data from cache line
    // offset is always 0 for L2 cache
    memcpy(data, &(L2Cache[index*BLOCK_SIZE + offset]), BLOCK_SIZE);
    time += L2_READ_TIME;
  }

  if (mode == MODE_WRITE) { // write data from cache line
    // offset is always 0 for L2 cache
    memcpy(&(L2Cache[index*BLOCK_SIZE + offset]), data, BLOCK_SIZE);
    time += L2_WRITE_TIME;
    Line->Dirty = 1;
  }
}
